...
(function($) {
    ...
    var scrollBar = $('.alumDetails .news .scroll-pane');
    setTimeout(function() {
        ...
        var timerId = setInterval(function() {
            scrollBarFlag = false;
            if (counter > $('#block-homealumninews article').length) {
                counter = 0;
            }

            var acticleHeight = $('#block-homealumninews article').outerHeight()
            scrollBar.data('jsp').scrollTo(0, acticleHeight * counter);
            counter++
            setTimeout(function() {
                scrollBarFlag = true
            }, 200)
    ...
    //$('.alumDetails .news .scroll-pane').jScrollPane();
    ...
    window.onload = function() {
        var arr = [];
        var temp = '';
        $('.pollRes').each(function() {
            var label = $(this).attr('data-label');
            var perc = $(this).attr('data-perc');

            arr.push({ label: label, y: perc });

        });
        JSON.stringify({ array: arr });
        //console.log(arr);
        ...
        $("#chartContainer").CanvasJSChart({
            ...
            data: [{
                ...
                dataPoints: arr
                    /*[
                            { label: "Collaborate on a Project", y: 36 },
                            { label: "Invite faculty to conduct workshop", y: 9 },
                            { label: "Refer a Recruiter", y: 9 },
                            { label: "Co-develop a case study", y: 18 },
                            { label: "Take a classroom session", y: 27 },
                        ]*/
            }]
        });
    }
